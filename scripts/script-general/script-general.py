...
for index, item in enumerate(data["mSource"]["mTerms"]["Array"]):
...
#        break
    ...
    try:
        chat_completion = client.chat.completions.create(
            model='gpt-4o',
            messages=[
                {'role': 'system', 'content': 'Translate the following text from Russian to Ukrainian. Maintain the tone, style, and nuances of the original text. Preserve the atmosphere and emotional depth typical of Disco Elysium. Adapt slang and idiomatic expressions to sound natural in Ukrainian. If a direct translation results in an awkward phrase, feel free to rephrase it to fit the context. Use correct Ukrainian grammar and stylistic conventions. Think twice before translation. If an input to you would be without ant character, or with space, ignore that input, dont answer. This is techical texts for item descriptions, names, titles, menu points, etc, so be presise'},
